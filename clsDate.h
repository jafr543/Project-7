...
public:
   ...
   void Print()
	...
	{
	    short Y = Date.GetYear();
	    short M = Date.GetMonth();
	    short D = Date.GetDay();
	...
	{
	    short Y = Date.GetYear() + Decade * 10;
		Date.SetYear(Y);
		return Date;
	}
	...
	static clsDate DecreaseDateByOneDay(clsDate Date)
	{
		...
		if (Date.GetDay() == 1)
		{
			...
			{
				M--;
				Date.SetDay(NumberOfDaysInAMonth(M,Y));
			}
		}
		else
		{
	...
	{
	    short Y = Date.GetYear() - Decades * 10;
		Date.SetYear(Y);
		return Date;
	}
